...
For the deployed app, push `app.py`, `collect_data.py`, and the whole `mlbb_predictor/` package alongside the existing model/data files. Reboot the deployed app if needed to load the new worker code. Optional collected profile snapshots can be backed up separately; a fresh deployment refreshes them automatically. Offline UI tests intentionally disable network checks.
...
4. Predictions, player ratings, collected matches, regular-season standings, team/player favorite picks, and meta fit refresh together after successful collection. Newly observed played heroes also appear in the draft and tier editors. An open page checks for newly published data every ten seconds, including profile and tier configuration changes.
5. Official team profiles refresh daily even when no matches are due. Registered players, roles, coaches/staff, summaries, and achievements come from each team's official Season 18 page. Profile errors retry hourly independently of match-source errors, preserve each team's last-good data, and appear in **Data collection**.
...
Your season tier grades remain editable settings; collection adds discovered heroes but does not invent tier grades. The prediction five follows the latest verified collected lineup when all five belong to the official roster. If that cannot be resolved, a roster with exactly one registered player per role can supply the five; otherwise the saved prediction lineup remains with a visible warning. The official source does not distinguish starters and substitutes, so **Main** means used for prediction, not a permanent roster designation. An overlapping prediction lineup after an unresolved transfer prevents that matchup from being predicted. Player aliases preserve existing ratings across official spelling changes and transfers.

Standings are recomputed from the same collected regular-season results used by **Collected matches**, including games with incomplete drafts. Only complete BO3 series count; playoff results are excluded. The displayed rank orders series wins, game difference, then game wins, with shared ranks for equal records. These are collected standings, may lag the source under the 24-hour rule, and do not apply official tiebreakers. The old August 28 standings file no longer drives the app.

Profiles are atomically saved to `data/processed/team_profiles.json`, with timestamps and retry status in `data/processed/profile_collection_status.json`. Curated `config/mpl_ph_teams.json` remains the fallback. No raw profile pages or credentials are needed at runtime. Profile source/season rollover requires an explicit configuration/code update; a different season is rejected. S13/S14 context remains historical and labeled accordingly.
...
- **MPL PH team profiles:** inspect automatically refreshed Season 18 rosters, individual player Elo, Season 18 game records, roles, registered reserves, staff, achievements, calculated standings, and cumulative team/player most-used picks this season. The inspector and standings table use the same calculated record. Historical S13 picks are removed from the interface.
...
2. For each starter, count every verified Season 18 player/hero appearance, even if they played for another team this season. Counts are cumulative, without a rolling window or older-season picks. More frequently played heroes receive more weight. Departed players' pools are not inherited by the old team.
...
The automatic evidence table shows the current players, top heroes, S/A pick share, sample sizes, excluded games, history dates, and each player's share of the meta Elo adjustment. It displays the top three heroes per player but calculates from their entire collected Season 18 pool. Bans, invalid drafts, and incomplete player-to-hero mappings are excluded. Older-patch history can still be present. The scores describe historical tendencies, not a guaranteed future draft or a statistically validated increase in win rate.

The evidence section explicitly labels **Most-played heroes · S18** and shows the collected cutoff. Use **Verify a player's Season 18 picks** to inspect every counted game, hero, opponent, and match-source link. No older-season pool is used when a player has no Season 18 picks; that player's meta contribution stays neutral. The source audit on September 9 checked all 12 Liquid/ONIC series and found the saved player/hero mappings matched MLDB. Sanji's 13 recorded games through September 6 contain Novaria 5, Eudora 4, Selena 3, and Valentina 1, with zero Yve picks. These are calculated audit results, not configured hero weights or counts.
...
Team-favorite heroes, player most-used picks, draft comfort, and automatic meta evidence use **all collected Season 18 games**. A new verified Paquito appearance increases a player's existing count by exactly one; repeat imports are deduplicated. Invalid drafts and incomplete player mappings are excluded, and their missing-data counts are visible. S17 outcomes still train the Elo baseline, but S17/S13 picks do not enter the season pick tables or meta pools. Five favorite heroes are suggested beside each draft, ten team favorites are shown on the profile, and every counted hero is used for scoring. Expand **All player hero counts this season** for the full player list.

**Collected matches** is split into weekly tables, newest first, with an **All weeks / Week N** filter. Event week numbers are extracted when available; older seed rows use calendar weeks anchored to the season's collected opening week. **Lane-specific meta assignments** has a **Filter by role** selector for EXP, Gold, Jungle, Mid, and Roam. These filters affect display only; they do not filter the games used for Elo or season totals.

The September 9, 2026 catch-up collected 22 additional games, bringing the saved snapshot to 21 series / 51 Season 18 games through September 6. All 51 games have verified lineups and usable picks; combined with S17, the active Elo model has 220 games. From these results Aurora is fourth at 2–3, and Liquid first at 5–1. Match points equal series wins; net game wins are game wins minus losses. Omega and TNC currently share the same calculated record and rank because official tiebreakers are not inferred. These numbers document that verified snapshot; the app recalculates them from rows on every data revision.

Deploy the updated `app.py`, the complete `mlbb_predictor/` package, and the latest `data/processed/live_snapshot.json` together to publish the updated interface and already-collected results. A page showing “standings snapshot is dated 2026-08-28” is still running the older interface. Source checks and future collections need a running server; source delays and queued results are shown in the app rather than presented as current standings.
...
The seed import had only four published player/hero entries per side for two Twisted Minds–ONIC games on August 28. Collection on August 30 recovered complete source records, so both now contribute to Elo and favorites in the live snapshot. The seed files preserve the original missing-data flags for auditability. Nine older S17 drafts contain duplicate/missing heroes and are excluded from pick counts only; their complete lineup results still update Elo. Each team profile shows the Season 18 series count, usable-game denominator, and missing draft count. Missing drafts do not count as zero-pick games. Absent players are never inferred from the current roster.

Curated fallback team information is stored in `config/mpl_ph_teams.json`; refreshed official profiles live in `data/processed/team_profiles.json`. Every prediction player is looked up individually. Players without a verified game start at a neutral 1500 rating and are visibly labeled. Explicit source-name aliases preserve continuity (including Domeng/Domengkite, Kyle/KyleTzy, Flap/FlapTzy, and Teddyqt/Teddy).

The current-team profiles link back to the [official MPL Philippines team directory](https://ph-mpl.com/teams). Standings update automatically from collected results, with the loaded-result cutoff displayed.
...
6. At prediction time, the app averages the five players resolved from official rosters and the latest complete collected lineup, using the saved five when ambiguous. New or previously unseen players remain at 1500.
...
- Season favorite-pick totals may span earlier rosters within Season 18 and contain flagged source gaps. They describe team tendencies, not individual mastery.
